...
p1 <- ggplot(data = D_shannon1) + geom_boxplot(aes(x = PD,y = value,fill = PD), width = 0.2) + labs(x = "",y = "shannon diversity") + theme_bw() + theme(axis.text.x = element_text(color = "black"),
  strip.background = element_rect(fill = "white"),
  panel.grid.major.y = element_blank(),
  panel.grid.minor.y = element_blank(),
...
p2 <- ggplot(data = D_simpson1) + geom_boxplot(aes(x = PD,y = value,fill = PD), width = 0.2) + labs(x = "",y = "simpson diversity") + theme_bw() + theme(axis.text.x = element_text(color = "black"),
  strip.background = element_rect(fill = "white"),
  panel.grid.major.y = element_blank(),
  panel.grid.minor.y = element_blank(),
  panel.grid.major.x = element_line(color = "white", size = 0.4),
...
```{r}
fig3 <- ggpubr::ggarrange(p1,p2,ncol = 2,common.legend = T)
ggsave(fig3, filename = "./stage2/figure3.pdf",width = 12, height = 8, units = "in")
...
CV_pred <- function(df, y = y){
  ...
  for(i in 1:n){
    X_train <- df[-i, ]
    X_test <- matrix(df[i,], nrow =  1)
    
    y_train <- y[-i]
    y_test <- y[i]
    
    lasso_fit <- cv.glmnet(X_train, y_train, family = "binomial")
    pred[i,1] <- predict(object = lasso_fit, newx = X_test, type = "response", s = "lambda.1se") 
    
    svm_fit <- svm(X_train, y = y_train, kernel = "radial", probability = TRUE, type = "C-classification")
    pred_svm <- predict(object = svm_fit, X_test, probability = TRUE)
    pred[i,2] <- attr(pred_svm, "probabilities")[1,"1"]
    
    rf_fit <- randomForest(x = X_train, y = y_train, ntree = 500)
    pred[i,3] <- predict(rf_fit, X_test, type="response")
    
    pred[i,4] <- y_test
  }
  
  colnames(pred) <- c("lasso", "svm", "random forest", "actual")
  return(pred)
}
```

```{r}
rocpdf<- function(df, method = "random forest", gd = "actual", model_name = method){
...
CV_ROC <- function(ps3){
  ...
  idx_exc <- c("18-093-t-0", "18-099-t-0", "18-264-t-0", "18-261-t-0", "19-017-t-0", "18-033-t-0", "18-087-t-0", "18-098-t-0", "18-103-t-0", "18-122-t-0", "18-137-t-0", "18-149-t-0", "18-255-t-0", "18-256-t-0", "17-085-t-0", "18-282-t-0", "19-001-t-0", "19-201-t-0", "18-108-t-0")
  
  ps_test <- subset_samples(ps3, !(sample_names(ps3) %in% idx_exc))
  n <- nrow(ps_test@sam_data)
  
  psphy <- tax_glom(ps_test, taxrank = "Phylum")
  psphy_ra <- transform_sample_counts(psphy, function(x){x / sum(x)})
  psphy_df <- psphy_ra@otu_table
  colnames(psphy_df) <- psphy@tax_table[,2]
  psphy_df_log <- trans_comp(psphy_df, eps = 1e-5, method = "clr")
  psphy_df_asin <- trans_comp(psphy_df, eps = 1e-5, method = "asin")
  
  psord <- tax_glom(ps_test, taxrank = "Order")
  psord_ra <- transform_sample_counts(psord, function(x){x / sum(x)})
  psord_df <- psord_ra@otu_table
  colnames(psord_df) <- psord@tax_table[,4]
  psord_df_log <- trans_comp(psord_df, eps = 1e-5, method = "clr")
  psord_df_asin <- trans_comp(psord_df, eps = 1e-5, method = "asin")
  
  psfam <- tax_glom(ps_test, taxrank = "Family")
  psfam_ra <- transform_sample_counts(psfam, function(x){x / sum(x)})
  psfam_df <- psfam_ra@otu_table
  colnames(psfam_df) <- psfam@tax_table[,5]
  psfam_df_log <- trans_comp(psfam_df, eps = 1e-5, method = "clr")
  psfam_df_asin <- trans_comp(psfam_df, eps = 1e-5, method = "asin")
  
  psgen <- tax_glom(ps_test, taxrank = "Genus")
  psgen_ra <- transform_sample_counts(psgen, function(x){x / sum(x)})
  psgen_df <- psgen_ra@otu_table
  colnames(psgen_df) <- psgen@tax_table[,6]
  psgen_df_log <- trans_comp(psgen_df, eps = 1e-5, method = "clr")
  psgen_df_asin <- trans_comp(psgen_df, eps = 1e-5, method = "asin")
  
  psotu_ra <- transform_sample_counts(ps_test, function(x){x / sum(x)})
  psotu_df <- psotu_ra@otu_table
  colnames(psotu_df) <- paste(psotu_ra@tax_table[,6], paste("o.", 1:ncol(psotu_df), sep = ""), sep = ":")
  psotu_df_log <- trans_comp(psotu_df, eps = 1e-5, method = "clr")
  psotu_df_asin <- trans_comp(psotu_df, eps = 1e-5, method = "asin")
  
  X_list <- list(psphy_df_log, psphy_df_asin, psord_df_log, psord_df_asin, psfam_df_log, psfam_df_asin, psgen_df_log, psgen_df_asin, psotu_df_log, psotu_df_asin)
  
  y <- as.numeric(as.factor(ps_test@sam_data$PD)) - 1
  
  CV_pred_func <- purrr::partial(CV_pred, y = y)
  
  pred_list <- lapply(X_list, CV_pred_func)
...
```{r}
library(ROCR)
roc_method <- function(mat){
  
  mat <- as.data.frame(mat)
  n <- nrow(mat)
  res_roc <- matrix(0, nrow = 1, ncol = 3)
  roc_lasso <- rocpdf(mat, method = "lasso")
  roc_svm <- rocpdf(mat, method = "svm")
  roc_rf <- rocpdf(mat, method = "random forest")

  res_roc <- rbind(roc_lasso$df, roc_svm$df, roc_rf$df)
  res_auc <- c(roc_lasso$auc, roc_svm$auc, roc_rf$auc)
  
  return(list(df = res_roc, auc = res_auc))
}
```

```{r}
roc_res <- lapply(pred_list, roc_method)
```
...
X1_nutri <- as.data.frame(as.matrix(ps_test@sam_data[,c(4,153:177)]))
for(i in 1:ncol(X1_nutri)){
  X1_nutri[,i] <- as.numeric(X1_nutri[,i])
}
Z_add <- as.data.frame(X1_nutri)
Z_add$Pratio <- Z_add[,"Tot_Protein_g_per_day"] * 17 / Z_add[,"Tot_Energy_without_Dietary_Fibre_kJ_per_day"]
Z_add$Fratio <- Z_add[,"Tot_Fat_g_per_day"] * 37 / Z_add[,"Tot_Energy_without_Dietary_Fibre_kJ_per_day"]
Z_add$Cratio <- Z_add[,"Tot_Avail_Carbs_g_per_day"] * 17 / Z_add[,"Tot_Energy_without_Dietary_Fibre_kJ_per_day"]
Z_add <- Z_add[,-2]
Z_add <- as.matrix(Z_add)
#Z_add <- scale(Z_add)
Z <- Z_add
```

```{r}
temp1 <- InfoGain_cont(Z[,"Tot_Protein_g_per_day"], y)
temp2 <- InfoGain_cont(Z[,"Tot_Fat_g_per_day"], y)
temp3 <- InfoGain_cont(Z[,"Tot_Added_Sugars_g_per_day"], y)
temp4 <- InfoGain_cont(Z[,"Tot_B12_ug_per_day"], y)
temp5 <- InfoGain_cont(Z[,"Tot_Potas_mg_per_day"], y)
temp6 <- InfoGain_cont(Z[,"Tot_Fibre_g_per_day"], y)
temp7 <- InfoGain_cont(Z[,"Tot_Avail_Carbs_g_per_day"], y)
temp8 <- InfoGain_cont(Z[,"Pratio"], y)
temp9 <- InfoGain_cont(Z[,"Fratio"], y)
temp10 <- InfoGain_cont(Z[,"Cratio"], y)
```



```{r}
CV_split <- function(psfam_df_log){
  split_var <- c("Tot_Protein_g_per_day", "Tot_Fat_g_per_day", "Tot_Added_Sugars_g_per_day", "Tot_Fibre_g_per_day", "Tot_Avail_Carbs_g_per_day", "Pratio", "Fratio", "Cratio")
  split_num <- c(temp1$s, temp2$s, temp3$s, temp6$s, temp7$s, temp8$s, temp9$s, temp10$s )

  X <- psfam_df_log
  n <- nrow(X)
  
  pred_split <- matrix(0, nrow = n, ncol = 9)
  ...
  for(i in 1:n){
    ...
    for(j in 1:8){
      idx1 <- which(Z_train[,split_var[j]] > split_num[j])
      
      X_train1 <- X_train[idx1,]
      X_train2 <- X_train[-idx1,]
      y_train1 <- y_train[idx1]
      y_train2 <- y_train[-idx1]
      
      rf1 <- randomForest(x = X_train1, y = y_train1, ntree = 500)
      rf2 <- randomForest(x = X_train2, y = y_train2, ntree = 500)
      
      sel <- Z_test[split_var[j]] > split_num[j]
      
      if(sel){
        pred_temp <- predict(rf1, X_test)
      }else{
        pred_temp <- predict(rf2, X_test)
      }
      
      pred_split[i,j] <- pred_temp
    }
    pred_split[i,9] <- y_test
  }
  return(pred_split)
}
```
...
```{r}
colnames(pred_split) <- c("Protein", "Fat", "Sugar", "Fiber", "Carb", "Pratio", "Fratio", "Cratio", "actual")
```
...
df_roc_tax <- rbind(df_phy, df_ord, df_fam, df_gen, df_asv)
df_auc_tax <- rbind(roc_res[[1]]$auc, roc_res[[3]]$auc, roc_res[[5]]$auc, roc_res[[7]]$auc, roc_res[[9]]$auc)
```


```{r}
caption_lasso <- paste0("AUC: ", round(df_auc_tax[1,1], 2),"(Phylum), ", round(df_auc_tax[2,1], 2),"(Order), ", round(df_auc_tax[3,1], 2), "(Family), ",round(df_auc_tax[4,1], 2), "(Genus), ",round(df_auc_tax[5,1], 2), "(ASV)")

caption_svm <- paste0("AUC: ", round(df_auc_tax[1,2], 2),"(Phylum), ", round(df_auc_tax[2,2], 2),"(Order), ", round(df_auc_tax[3,2], 2), "(Family), ",round(df_auc_tax[4,2], 2), "(Genus), ",round(df_auc_tax[5,2], 2), "(ASV)")

captionrf <- paste0("AUC: ", round(df_auc_tax[1,3], 2),"(Phylum), ", round(df_auc_tax[2,3], 2),"(Order), ", round(df_auc_tax[3,3], 2), "(Family), ",round(df_auc_tax[4,3], 2), "(Genus), ",round(df_auc_tax[5,3], 2), "(ASV)")
```

```{r}
df_svm <- df_roc_tax[df_roc_tax$model == "svm",]
df_svm$tax <- factor(df_svm$tax, levels = c("Phylum","Order","Family","Genus","ASV"))
fig_svm <- ggplot(df_svm) + geom_path(aes(x = x,y = y,color = tax),size = 1) + theme(plot.title = element_text(size = 10), legend.title = element_blank()) + theme_bw() + geom_abline(slope = 1,intercept = 0) + labs(x = "1 - Specificity", y = "Sensitivity", caption = caption_svm, title = "LOOCV-ROC curve of SVM model on different taxanomic level")+ scale_color_manual(values = c("#8DD3C7", "#BEBADA","#FB8072","#80B1D3","#FDB462","#B3DE69","#FCCDE5"))
```

```{r}
df_rf <- df_roc_tax[df_roc_tax$model == "random forest",]
df_rf$tax <- factor(df_rf$tax, levels = c("Phylum","Order","Family","Genus","ASV"))
fig_rf <- ggplot(df_rf) + geom_path(aes(x = x,y = y,color = tax),size = 1) + theme(plot.title = element_text(size = 10), legend.title = element_blank()) + theme_bw() + geom_abline(slope = 1,intercept = 0) + labs(x = "1 - Specificity", y = "Sensitivity", caption = captionrf, title = "LOOCV-ROC curve of Random Forest model on different taxanomic level")+ scale_color_manual(values = c("#8DD3C7", "#BEBADA","#FB8072","#80B1D3","#FDB462","#B3DE69","#FCCDE5"))
```
...
roc_method <- function(mat){
  ...
  mat <- as.data.frame(mat)
  n <- nrow(mat)
  res_roc <- matrix(0, nrow = 1, ncol = 8)
  roc_protein <- rocpdf(mat, method = "Protein")
  roc_fat <- rocpdf(mat, method = "Fat")
  roc_sugar <- rocpdf(mat, method = "Sugar")
  roc_fiber <- rocpdf(mat, method = "Fiber")
  roc_carb <- rocpdf(mat, method = "Carb")
  roc_pr <- rocpdf(mat, method = "Pratio")
  roc_fr <- rocpdf(mat, method = "Fratio")
  roc_cr <- rocpdf(mat, method = "Cratio")

  res_roc <- rbind(roc_protein$df, roc_fat$df, roc_sugar$df,roc_fiber$df,roc_carb$df,roc_pr$df,roc_fr$df,roc_cr$df)
  res_auc <- c(roc_protein$auc, roc_fat$auc, roc_sugar$auc,roc_fiber$auc,roc_carb$auc,roc_pr$auc,roc_fr$auc,roc_cr$auc)
...
```{r}
roc_split <- roc_method(pred_split)
```

```{r}
df_temp <- df_rf[df_rf$tax == "Family",]
df_temp <- df_temp[,-c(3,4)]
df_temp$model <- "Microbiome"
```

```{r}
roc_split_df <- rbind(roc_split$df, df_temp)
roc_split_df$model <- factor(roc_split_df$model, levels = c("Protein", "Fat", "Sugar", "Fiber", "Fratio", "Cratio","Carb", "Pratio", "Microbiome"))
```

```{r}
caption_split <- paste0("AUC: ", round(df_auc_tax[3,3], 2),"(Microbiome, Family), ", round(roc_split$auc[1], 2),"(Protein), ", round(roc_split$auc[2], 2), "(Fat), ",round(roc_split$auc[5], 2), "(Carb), ",round(roc_split$auc[4], 2), "(Fiber), \n", round(roc_split$auc[3], 2), "(Sugar), ",round(roc_split$auc[6], 2), "(Pratio), ",round(roc_split$auc[7], 2), "(Fratio), ",round(roc_split$auc[8], 2), "(Cratio)")
```

```{r}
fig_split <- ggplot(roc_split_df) + geom_path(aes(x = x,y = y,color = model),size = 1) + theme(plot.title = element_text(size = 10), legend.title = element_blank()) + theme_bw() + geom_abline(slope = 1,intercept = 0) + labs(x = "1 - Specificity", y = "Sensitivity", caption = caption_split, title = "LOOCV-ROC curve of 2 stage Random Forest model \n with different nutrients as splitting node")+ scale_color_manual(values = c("#FB8072", "#BEBADA","#8DD3C7","#80B1D3","#FDB462","#B3DE69","#FCCDE5","#E5C494", "#FC8D62"))
```


```{r}
fig_split2 <- ggplot(roc_split_df) + geom_path(aes(x = x,y = y,color = model),size = 1) + theme(plot.title = element_text(size = 10), legend.title = element_blank()) + theme_bw() + geom_abline(slope = 1,intercept = 0) + labs(x = "1 - Specificity", y = "Sensitivity", caption = caption_split, title = "LOOCV-ROC curve of 2 stage Random Forest model \n with different nutrients as splitting node")+ scale_color_manual(values = c("#D9D9D9","#C6DBEF","#E5F5E0","#EFEDF5","#FEE6CE", "#E7E1EF","#A50F15","#08519C", "#006D2C"))
```
...
```{r}
X <- phyloseq::tax_glom(psra, taxrank = "Family")
y <- psra@sam_data$PD
```

```{r}
x <- as.matrix(X@otu_table)
y <- as.numeric(as.factor(y)) - 1
p1 <- adonis2(x~y, permutations = 9999)
p1
```

```{r}

ps01_sub2 <- subset_samples(ps0, latent == 2)

seq <- as.list(colnames(ps01_sub2@otu_table))

seq_name <- paste(ps01_sub2@tax_table[,5], 1:length(seq), sep = "")

write.fasta(seq, seq_name, "sample77.fasta")
```

```{r}
asv_tab <- as.data.frame(ps01_sub2@otu_table)
colnames(asv_tab) <- seq_name

asv_tab <- t(as.matrix(asv_tab))

write.table(asv_tab, "sample77.txt", sep = "\t", quote = F)
```

```{r}
library(readr)
pathway_abd <- read_tsv("path_abun_unstrat_descrip.tsv")
```

```{r}
path_info <- pathway_abd[,1:2]
path_tab <- as.matrix(round(pathway_abd[,3:79]))
rownames(path_tab) <- unlist(path_info[,1])
```
...
```{r}
path_aldex <- aldex(path_tab, ps0@sam_data$PD[latent == 2], mc.samples=1024, test="t",
                    effect=TRUE,include.sample.summary=FALSE, denom="all", verbose=FALSE)
df_ps <- path_aldex[,c("diff.btw","wi.ep","we.ep", "we.eBH", "wi.eBH")]
```

```{r}
df_ps$descrip <- unlist(path_info[,2])
```

```{r}
color = rep("1",nrow(df_ps))
color[(abs(df_ps$diff.btw)>0.5)&(df_ps$wi.ep<0.05)] = "2"
df_anno = df_ps[(abs(df_ps$diff.btw)>0.5)&(df_ps$wi.ep<0.05),]
ggplot(df_ps) + geom_point(aes(x = diff.btw, y = -log(wi.ep, base = 2), color = color)) + labs(x = "fold change", y = "-log(p)") + theme_classic() + scale_color_manual(values = c("gray30","red")) + geom_text(data = df_anno,aes(x = diff.btw - 0.01,y=-log(wi.ep, 2)-0.01),label = rownames(df_anno), size = 3) + theme(legend.position="none")
```

```{r}
write.csv(df_ps, "pathway_77.csv")
```
