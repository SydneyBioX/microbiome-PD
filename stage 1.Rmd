...
sample_info$Treatment[sample_info$Treatment == "Duodopa"] = "LCIG"
```
...
```{r}
Bristol.new <- readxl::read_xlsx("Project Database - Advanced Therapies Cohort.xlsx")
Bristol.data0 <- Bristol.new[,c("Neurogenetics ID","PD Status","Cohort Identifier","Intervals","Bristol Score")]
```

```{r}
Bristol.data <- Bristol.data0[!is.na(Bristol.data0$`Bristol Score`),]
Bristol.data <- Bristol.data[Bristol.data$Intervals != 7,]
id0 <- Bristol.data$`Neurogenetics ID`[1]
Cohort0 <- Bristol.data$`Cohort Identifier`[1]
for (i in 2:nrow(Bristol.data)){
  if (is.na(Bristol.data$`Neurogenetics ID`[i])){
    Bristol.data$`Neurogenetics ID`[i] = id0
  }else{
    id0 <- Bristol.data$`Neurogenetics ID`[i]
  }
  if(is.na(Bristol.data$`Cohort Identifier`[i])){
    Bristol.data$`Cohort Identifier`[i] = Cohort0
  }else{
    Cohort0 <- Bristol.data$`Cohort Identifier`[i]
  }
}
```
...
```
  ...
  In this study, we first consider 88 samples with 78 Parkinson’s disease patients and 10 controls. After processing, we get total `r length(sample_names(ps))` samples, with 4 replicates, say, 18-033-t-0-rpt2, 18-033-t-0-rpt3, 18-069-t-0-rpt2, 18-069-t-0-rpt3.
...
```{r}
ps3ra_sub <- subset_samples(ps3ra, !is.na(Bristol.Score))
ps3ra_fam <- tax_glom(ps3ra_sub, "Family")

GPUF <- phyloseq::distance(ps3ra_fam, method = 'bray')

adTPPD_noconf_OTUs <- adonis2(GPUF ~ sample_data(ps3ra_sub)[["Bristol.Score"]] + sample_data(ps3ra_sub)[["Treatment"]] + sample_data(ps3ra_sub)[["t"]] , perm = 9999, by = "margin")
adTPPD_noconf_OTUs
```
...
```{r}
clinical <- as.data.frame(ps3@sam_data)[,1:176]

clinical <- clinical[clinical$t == 0,]
clinical <- as.data.frame(clinical)
clinical <- clinical[-c(4,5,9,10),]
test_mat <- data.frame(var_name = "", tp = 1, tstat = 0, tdf = 0, wkp = 1, wkstat = 0, wep = 1, westat = 0, wedf = 0, shapt_PD = 1, shapt_HC = 1, bartt = 1, barttstat = 0)

test_mat1 <- data.frame(var_name = "", tp = 1, tstat = 0, tdf = 0, wkp = 1, wkstat = 0, wep = 1, westat = 0, wedf = 0, shapt_PD = 1, shapt_HC = 1, bartt = 1, barttstat = 0)

for(i in 1:ncol(clinical)){
  
  temp <- unlist(clinical[,i])
  
  temp1 <- as.numeric(as.character(temp))
  
  temp_PD <- unlist(clinical[clinical$PD == "yes",i])
  temp_PD1 <- as.numeric(as.character(temp_PD))
  Treatment <- unlist(clinical$Treatment[clinical$PD == "yes"])
  
  if(sum(is.na(temp1)) < 10){
    
    fct_check <- length(unique(temp))
    
    if(fct_check > 7){
      
      res_test <- data.frame(var_name = "", tp = 1, tstat = 0, tdf = 0, wkp = 1, wkstat = 0, wep = 1, westat = 0, wedf = 0, shapt_PD = 1, shapt_HC = 1, bartt = 1, barttstat = 0)
      
      res_test[1,1] <- colnames(clinical)[i]
    
      res_t <- t.test(temp1 ~ clinical$PD, var.equal = T)
      
      res_test[1,2] <- res_t$p.value
      res_test[1,3] <- res_t$statistic
      res_test[1,4] <- res_t$parameter
      
      res_wk <- wilcox.test( temp1 ~ clinical$PD)
      res_test[1,5] <- res_wk$p.value
      res_test[1,6] <- res_wk$statistic
      
      res_we <- t.test(temp1 ~ clinical$PD)
      res_test[1,7] <- res_we$p.value
      res_test[1,8] <- res_we$statistic
      res_test[1,9] <- res_we$parameter
      
      res_shap_PD <- shapiro.test(temp1[clinical$PD == "yes"])
      res_shap_HC <- shapiro.test(temp1[clinical$PD == "no"])
      res_test[1,10] <- res_shap_PD$p.value
      res_test[1,11] <- res_shap_HC$p.value
      
      
      res_bart <- bartlett.test(temp1~clinical$PD)
      
      res_test[1,12] <- res_bart$p.value
      res_test[1,13] <- res_bart$statistic
      
      test_mat <- rbind(test_mat, res_test)
    }
    
    
  }
  
  if(sum(is.na(temp_PD1)) < 7){
    
    fct_check1 <- length(unique(temp1))
    
    if(fct_check1 > 7){
      
      res_test1 <- data.frame(var_name = "", tp = 1, tstat = 0, tdf = 0, wkp = 1, wkstat = 0, wep = 1, westat = 0, wedf = 0, shapt_PD = 1, shapt_HC = 1, bartt = 1, barttstat = 0)
      
      res_test1[1,1] <- colnames(clinical)[i]
    
      res_t <- t.test(temp_PD1 ~ Treatment, var.equal = T)
      
      res_test1[1,2] <- res_t$p.value
      res_test1[1,3] <- res_t$statistic
      res_test1[1,4] <- res_t$parameter
      
      res_wk <- wilcox.test( temp_PD1 ~ Treatment)
      res_test1[1,5] <- res_wk$p.value
      res_test1[1,6] <- res_wk$statistic
      
      res_we <- t.test(temp_PD1 ~ Treatment)
      res_test1[1,7] <- res_we$p.value
      res_test1[1,8] <- res_we$statistic
      res_test1[1,9] <- res_we$parameter
      
      res_shap_LCIG <- shapiro.test(temp_PD1[Treatment == "LCIG"])
      res_shap_DBS <- shapiro.test(temp_PD1[Treatment == "DBS"])
      res_test1[1,10] <- res_shap_LCIG$p.value
      res_test1[1,11] <- res_shap_DBS$p.value
      
      
      res_bart <- bartlett.test(temp_PD1~Treatment)
      
      res_test1[1,12] <- res_bart$p.value
      res_test1[1,13] <- res_bart$statistic
      
      
      test_mat1 <- rbind(test_mat1, res_test1)
    }
...
test_mat <- test_mat[-1,]
test_mat1 <- test_mat1[-1,]
```

```{r}
test_mat_p <- test_mat[,c(1,2,5,7,10,11,12)]
test_mat1_p <- test_mat[,c(1,2,5,7,10,11,12)]
```
...
```{r}
fig2 <- ggpubr::ggarrange(p1,p2,ncol = 1,common.legend = T)#grid.arrange(fig2)
ggsave(fig2, filename = "figure2_4.pdf",width = 8,height = 8,units = "in")
tiff(filename = "figure2_4.tiff" ,width=12, height=8, units="in", compression="lzw", res=150)
fig2
dev.off()
```

```{r}
colnames(Bristol.data1) <- c("id","PD","Treatment","t","Bristol score","name")
Bristol.data1$Treatment[Bristol.data1$Treatment == 1] = "LCIG"
Bristol.data1$Treatment[Bristol.data1$Treatment == 2] = "DBS"
Bristol.data1$Treatment[Bristol.data1$Treatment == 3] = "Control"
Bristol.data1$PD[Bristol.data1$PD == 1] = "yes"
Bristol.data1$PD[Bristol.data1$PD == 0] = "no"
p_Bristol <- ggplot(data = Bristol.data1) +  geom_point(aes(x = t, y = `Bristol score`, color = Treatment)) + geom_line(aes(x = t, y =  `Bristol score`, color = Treatment, group = id)) + theme(axis.text.x = element_text(angle = 45)) + facet_wrap(~Treatment)  + theme_bw() + labs(x = "",y = "Bristol Score") +
  theme(axis.text.x = element_text(color = "black"),
  strip.background = element_rect(fill = "white"),
  panel.grid.major.y = element_blank(),
  panel.grid.minor.y = element_blank(),
...
```{r}
ggsave(p_Bristol,filename = "Bristol0.pdf",width = 8,height = 4,units = "in")
```
...
eps = 1e-7
ps3 <- prune_samples(sample_names(ps2) != "18-203-t-2",ps2)
colnames(ps3@sam_data)[colnames(ps3@sam_data) == "Cohort"] = "Treatment"
ps3ra <- transform_sample_counts(ps3, function(x){x / sum(x)})

ps3ra0 <- subset_samples(ps3ra,sample_names(ps3ra) %in% Bristol.data1$name)
ps3ra0@sam_data$`Bristol Score` <- Bristol.data1$`Bristol score`[match(sample_names(ps3ra0),Bristol.data1$name)]

ps3ra_phy <- tax_glom(ps3ra0,taxrank = "Phylum")
#ps3ra_phy <- ps3ra0
ps3ra0@sam_data$Treatment[ps3ra0@sam_data$Treatment == "Longitudinal"] = "Control"
ps3ra0@sam_data$Treatment[ps3ra0@sam_data$Treatment == "Duodopa"] = "LCIG"
...
out.pcoa3ra <- ordinate(ps3ra_phy,  method = "PCoA", distance = "bray")
...
#ps3ra_gen <- tax_glom(ps3ra,taxrank = "Genus")


df = cbind(out.pcoa3ra$vectors[,1:3],ps3ra0@sam_data) %>% group_by(id) %>% arrange(t,.by_group = TRUE)
...
#df$`Bristol Score` = as.factor(df$`Bristol Score`)

p12 <- ggplot(df) + geom_point(aes(x = Axis.1, y = Axis.2, color = Treatment, shape = t, size = `Bristol Score`)) + geom_path(aes(x = Axis.1, y = Axis.2, group = id, colour = Treatment),  alpha = 0.5, size = 1) + labs(x = paste0("PC1: ", 100*round(var_explained[1],digits = 4), "%"), y = paste0("PC2: ", 100*round(var_explained[2],digits = 4), "%")) +  theme_classic()  + stat_ellipse(aes(x = Axis.1, y = Axis.2, group = Treatment, color = Treatment, lty = Treatment), level = 0.9) + scale_color_manual(values = brewer.pal(3, "Set2")) + ggtitle("A.") 

p13 <- ggplot(df) + geom_point(aes(x = Axis.1, y = Axis.3,  color = Treatment, shape = t, size = `Bristol Score`)) + geom_path(aes(x = Axis.1, y = Axis.3, group = id, colour = Treatment),  alpha = 0.5, size = 1) + labs(x = paste0("PC1: ", 100*round(var_explained[1],digits = 4), "%"), y = paste0("PC3: ", 100*round(var_explained[3],digits = 4), "%")) +  theme_classic()  + stat_ellipse(aes(x = Axis.1, y = Axis.3, group = Treatment, color = Treatment, lty = Treatment), level = 0.9) + scale_color_manual(values = brewer.pal(3, "Set2"))+ ggtitle("B.")

p23 <- ggplot(df) + geom_point(aes(x = Axis.2, y = Axis.3,  color = Treatment, shape = t, size = `Bristol Score`)) + geom_path(aes(x = Axis.2, y = Axis.3, group = id, colour = Treatment),  alpha = 0.5, size = 1) + labs(x = paste0("PC2: ", 100*round(var_explained[2],digits = 4), "%"), y = paste0("PC3: ", 100*round(var_explained[3],digits = 4), "%")) +  theme_classic()  + stat_ellipse(aes(x = Axis.2, y = Axis.3, group = Treatment, color = Treatment, lty = Treatment), level = 0.9) + scale_color_manual(values = brewer.pal(3, "Set2")) + ggtitle("C.")

#p12 <- ggplot(df) + geom_point(aes(x = Axis.1, y = Axis.2,  color = `Bristol Score`, shape = t), size = 2) + geom_path(aes(x = Axis.1, y = Axis.2, group = id), color = "gray30",  alpha = 0.5, size = 1) + labs(x = paste0("PC1: ", 100*round(var_explained[1],digits = 4), "%"), y = paste0("PC2: ", 100*round(var_explained[2],digits = 4), "%")) +  theme_classic()  + stat_ellipse(aes(x = Axis.1, y = Axis.2, group = Treatment, lty = Treatment), level = 0.9) + scale_color_gradientn(colours = brewer.pal(9,"OrRd"))+ ggtitle("A.")

#p13 <- ggplot(df) + geom_point(aes(x = Axis.1, y = Axis.3,  color = `Bristol Score`, shape = t), size = 2) + geom_path(aes(x = Axis.1, y = Axis.3, group = id), color = "gray30",  alpha = 0.5, size = 1) + labs(x = paste0("PC2: ", 100*round(var_explained[1],digits = 4), "%"), y = paste0("PC3: ", 100*round(var_explained[3],digits = 4), "%")) +  theme_classic()  + stat_ellipse(aes(x = Axis.2, y = Axis.3, group = Treatment, lty = Treatment), level = 0.9) + scale_color_gradientn(colours = brewer.pal(9,"OrRd"))+ ggtitle("B.")

#p23 <- ggplot(df) + geom_point(aes(x = Axis.2, y = Axis.3,  color = `Bristol Score`, shape = t), size = 2) + geom_path(aes(x = Axis.2, y = Axis.3, group = id), color = "gray30",  alpha = 0.5, size = 1) + labs(x = paste0("PC2: ", 100*round(var_explained[2],digits = 4), "%"), y = paste0("PC3: ", 100*round(var_explained[3],digits = 4), "%")) +  theme_classic()  + stat_ellipse(aes(x = Axis.2, y = Axis.3, group = Treatment, lty = Treatment), level = 0.9) + scale_color_gradientn(colours = brewer.pal(9,"OrRd"))+ ggtitle("C.")
```  
...
ggexport(fig3, filename = "BC_Phylum.pdf",width = 12, height = 8, units = "in")
tiff(filename = "BC_Phylum.tiff" ,width=12, height=8, units="in", compression="lzw", res=150)
fig3
dev.off()
```

```{r}
GPUF <- phyloseq::distance(ps3ra_phy, method = 'unifrac')
out.pcoa3ra <- ordinate(ps3ra_phy,  method = "PCoA", distance = GPUF)
evals <- out.pcoa3ra$values[,1]
#plot_ordination(ps3ra, out.pcoa3ra, color = "t",
#                  shape = "Cohort") +
#  labs(col = "Time", shape = "Cohort")+
...
```{r}
fig3 <- ggpubr::ggarrange(p12,p13,p23,ncol = 2,nrow = 2,common.legend = T)
ggexport(fig3, filename = "Unifrac_Phylum.pdf",width = 12, height = 8, units = "in")
tiff(filename = "Unifrac_Phylum.tiff" ,width=12, height=8, units="in", compression="lzw", res=150)
fig3
dev.off()
```

```{r}
GPUF <- phyloseq::distance(ps3ra_phy, method = 'wunifrac')
out.pcoa3ra <- ordinate(ps3ra_phy,  method = "PCoA", distance = GPUF)
evals <- out.pcoa3ra$values[,1]
#plot_ordination(ps3ra, out.pcoa3ra, color = "t",
#                  shape = "Cohort") +
...
```{r}
fig3 <- ggpubr::ggarrange(p12,p13,p23,ncol = 2,nrow = 2,common.legend = T)
ggexport(fig3, filename = "Wunifrac_Phylum.pdf",width = 12, height = 8, units = "in")
tiff(filename = "Wunifrac_Phylum.tiff" ,width=12, height=8, units="in", compression="lzw", res=150)
fig3
dev.off()
```
...
```{r}
ps3ra_phy <- tax_glom(ps3ra,taxrank = "Genus")
ps3ra_phy1 <- subset_samples(ps3ra, Treatment == "LCIG")

GPUF <- distance(ps3ra_phy1, method = 'wunifrac')
out.pcoa3ra1 <- ordinate(ps3ra_phy1,  method = "PCoA", distance = GPUF)
#out.pcoa3ra1 <- ordinate(ps3ra_phy1,  method = "PCoA", distance = "bray")
evals <- out.pcoa3ra1$values[,1]


df = cbind(out.pcoa3ra1$vectors[,1:3],ps3ra1@sam_data) %>% group_by(id) %>% arrange(t,.by_group = TRUE)

var_explained = out.pcoa3ra1$values$Relative_eig[1:3]
...
p12_1 <- ggplot(df) + geom_path(aes(x = Axis.1, y = Axis.2, group = id),alpha = 0.2,color = "gray30", size = 1) + labs(x = paste0("PC1: ", 100*round(var_explained[1],digits = 4), "%"), y = paste0("PC2: ", 100*round(var_explained[2],digits = 4), "%")) +  theme_classic()  + stat_ellipse(aes(x = Axis.1, y = Axis.2, group = t, color = t,lty = t), level = 0.9) + scale_color_manual(values = brewer.pal(4, "Set2")) + geom_text(aes(x = Axis.1, y = Axis.2, label = id, color = t)) + ggtitle("A. LCIG")

p13_1 <- ggplot(df) + geom_path(aes(x = Axis.1, y = Axis.3, group = id),alpha = 0.2,color = "gray30", size = 1) + labs(x = paste0("PC1: ", 100*round(var_explained[1],digits = 4), "%"), y = paste0("PC3: ", 100*round(var_explained[3],digits = 4), "%")) +  theme_classic()  + stat_ellipse(aes(x = Axis.1, y = Axis.3, group = t, color = t,lty = t), level = 0.9) + scale_color_manual(values = brewer.pal(4, "Set2")) + geom_text(aes(x = Axis.1, y = Axis.3, label = id, color = t)) + ggtitle("")

p23_1 <- ggplot(df) + geom_path(aes(x = Axis.2, y = Axis.3, group = id),alpha = 0.2,color = "gray30", size = 1) + labs(x = paste0("PC2: ", 100*round(var_explained[2],digits = 4), "%"), y = paste0("PC2: ", 100*round(var_explained[3],digits = 4), "%")) +  theme_classic()  + stat_ellipse(aes(x = Axis.2, y = Axis.3, group = t, color = t,lty = t), level = 0.9) + scale_color_manual(values = brewer.pal(4, "Set2")) + geom_text(aes(x = Axis.2, y = Axis.3, label = id, color = t)) + ggtitle("")
```


```{r}
ps3ra_phy2 <- subset_samples(ps3ra, Treatment == "DBS")
GPUF <- distance(ps3ra_phy2, method = 'wunifrac')
out.pcoa3ra2 <- ordinate(ps3ra_phy2,  method = "PCoA", distance = GPUF)
#out.pcoa3ra2 <- ordinate(ps3ra_phy2,  method = "PCoA", distance = "bray")
evals <- out.pcoa3ra2$values[,1]


df = cbind(out.pcoa3ra2$vectors[,1:3],ps3ra_phy2@sam_data) %>% group_by(id) %>% arrange(t,.by_group = TRUE)

var_explained = out.pcoa3ra2$values$Relative_eig[1:3]
...
p12_2 <- ggplot(df) + geom_path(aes(x = Axis.1, y = Axis.2, group = id),alpha = 0.2,color = "gray30", size = 1) + labs(x = paste0("PC1: ", 100*round(var_explained[1],digits = 4), "%"), y = paste0("PC2: ", 100*round(var_explained[2],digits = 4), "%")) +  theme_classic()  + stat_ellipse(aes(x = Axis.1, y = Axis.2, group = t, color = t,lty = t), level = 0.9) + scale_color_manual(values = brewer.pal(4, "Set2")) + geom_text(aes(x = Axis.1, y = Axis.2, label = id, color = t)) + ggtitle("B. DBS")

p13_2 <- ggplot(df) + geom_path(aes(x = Axis.1, y = Axis.3, group = id),alpha = 0.2,color = "gray30", size = 1) + labs(x = paste0("PC1: ", 100*round(var_explained[1],digits = 4), "%"), y = paste0("PC3: ", 100*round(var_explained[3],digits = 4), "%")) +  theme_classic()  + stat_ellipse(aes(x = Axis.1, y = Axis.3, group = t, color = t,lty = t), level = 0.9) + scale_color_manual(values = brewer.pal(4, "Set2")) + geom_text(aes(x = Axis.1, y = Axis.3, label = id, color = t)) + ggtitle("")

p23_2 <- ggplot(df) + geom_path(aes(x = Axis.2, y = Axis.3, group = id),alpha = 0.2,color = "gray30", size = 1) + labs(x = paste0("PC2: ", 100*round(var_explained[2],digits = 4), "%"), y = paste0("PC2: ", 100*round(var_explained[3],digits = 4), "%")) +  theme_classic()  + stat_ellipse(aes(x = Axis.2, y = Axis.3, group = t, color = t,lty = t), level = 0.9) + scale_color_manual(values = brewer.pal(4, "Set2")) + geom_text(aes(x = Axis.2, y = Axis.3, label = id, color = t)) + ggtitle("")
```



```{r}
p_1 <- ggpubr::ggarrange(p12_1,p13_1,p23_1,p12_2,p13_2,p23_2,nrow = 2,ncol = 3,common.legend = T) 
p_1
```


```{r}
ggexport(p_1, filename = "figure4_Wunifrac_OTU.pdf",width = 16, height = 8, units = "in")
tiff(filename = "figure4_Wunifrac_OTU.tiff" ,width=16, height=8, units="in", compression="lzw", res=150)
p_1
dev.off()
```
...
```{r}
fig3 <- ggpubr::ggarrange(p12,p13,p23,ncol = 2,nrow = 2,common.legend = T)
ggexport(fig3, filename = "figure4_Wunifrac_Phylum.pdf",width = fullpage, height = maxhi*0.75, units = "in")
tiff(filename = "figure4_Wunifrac_Phylum.tiff" ,width=12, height=8, units="in", compression="lzw", res=150)
fig3
dev.off()
```
...
# RDA
```{r}
ps30 <- subset_samples(ps3, (sample_names(ps3) != "19-201-t-0")&t == 0)
ps30_phy <- tax_glom(ps30,"Phylum")

otu_tab <- as.data.frame(ps30_phy@otu_table)
colnames(otu_tab) <- ps30_phy@tax_table[,2]
otu_tab <- decostand(otu_tab, method = 'hellinger')

clinical = sample_info
id_name <- rownames(ps30_phy@sam_data)
clinical <- clinical[(rownames(clinical) %in% id_name)&(clinical$t==0),]
miss_idx <- c()
for(i in 1:ncol(clinical)){
  flag <- sum(is.na(clinical[,i]))
  if(flag){
    miss_idx <- c(miss_idx,i)
  }
  if(class(clinical[,i]) == "character"){
    clinical[,i] <- as.factor(clinical[,i])
    if(length(levels(clinical[,i]))!=2){
      miss_idx <- c(miss_idx,i)
    }
  }
}
clinical <- clinical[,-miss_idx]

```

```{r}
clinical <- clinical[,-c(7:18,56:61)]
```

```{r}
clinical1 <- clinical[,-1]

clinical2 <- clinical1[,c("Age","Sex","BMI","Height (cm)","Weight (kg)","Constipation ID...170","Bristol Score","CRP","Total Cholesterol","HbA1c...262")]

out_rda <- capscale(otu_tab~., clinical2, distance = 'bray',add = T)
```

```{r}
out_rda.scaling2 <- summary(out_rda, scaling = 2)
out_rda.site <- data.frame(out_rda.scaling2$sites)[1:2]
out_rda.env <- data.frame(out_rda.scaling2$biplot)[1:2]
out_rda.species <- data.frame(out_rda.scaling2$species)[1:2]

explained <- round(out_rda.scaling2$concont$importance[2,1:2]*100,digits = 2)
```

```{r}
out_rda.site$PD <- clinical$PD
out_rda.env$group <- rownames(out_rda.env)
out_rda.species$group <- rownames(out_rda.species)

p <- ggplot(out_rda.site, aes(CAP1, CAP2)) +
geom_point(aes(color = PD)) +
scale_color_manual(values =  brewer.pal(2, "Set2"),breaks = c("yes","no"),labels = c("PD","Control")) +
...
labs(x = paste0("RDA1: ", explained[1], "%"), y = paste0("RDA1: ", explained[2], "%")) +
geom_vline(xintercept = 0, color = 'gray', size = 0.5) + 
geom_hline(yintercept = 0, color = 'gray', size = 0.5) +
geom_segment(data = out_rda.env, aes(x = 0,y = 0, xend = CAP1,yend = CAP2), arrow = arrow(length = unit(0.1, 'cm')), size = 0.3, color = 'blue', alpha = 0.2) +
geom_text(data = out_rda.env, aes(CAP1 * 1.1, CAP2 * 1.1, label = group), color = 'blue', size = 3) +
geom_segment(data = out_rda.species, aes(x = 0,y = 0, xend = CAP1,yend = CAP2), arrow = arrow(length = unit(0.1, 'cm')), size = 0.3, color = 'red', alpha = 0.2) +
geom_text(data = out_rda.species, aes(CAP1 * 1.1, CAP2 * 1.1, label = group), color = 'red', size = 2)
p
```

```{r}
ggexport(p, filename = "dbRDA_Phylum_BC.pdf",width = fullpage, height = maxhi*0.75, units = "in")
tiff(filename = "dbRDA_Phylum_BC.tiff" ,width=12, height=8, units="in", compression="lzw", res=150)
p
dev.off()
```
...
```{r}
Bristol.id <- c()
Bristol.data1 <- Bristol.data

for(i in unique(Bristol.data$Intervals)){
  if (i == 2){
    Bristol.data$Intervals[Bristol.data$Intervals == i] = 0
  }else if(i == 3){
    Bristol.data$Intervals[Bristol.data$Intervals == i] = 2
  }else if(i == 4){
    Bristol.data$Intervals[Bristol.data$Intervals == i] = 4
  }else if(i == 5){
    Bristol.data$Intervals[Bristol.data$Intervals == i] = 6
  }else if(i == 6){
    Bristol.data$Intervals[Bristol.data$Intervals == i] = 12
  }else if(i == 1){
    Bristol.data$Intervals[Bristol.data$Intervals == i] = -2
  }
}
Bristol.data1 <- Bristol.data[Bristol.data$Intervals<6,]

for(i in 1:nrow(Bristol.data1)){
  temp <- paste0(strsplit(Bristol.data1$`Neurogenetics ID`[i],"/")[[1]],collapse = "-")

  if(Bristol.data1$Intervals[i]>=0){
    temp <- paste(c(temp,"-t-",Bristol.data1$Intervals[i]),collapse = "")

  }else{
    temp <- paste(c(temp,"-t-minus2"),collapse = "")
  }
  Bristol.id[i] <- temp
}
Bristol.data1$names <- Bristol.id
```

```{r}
#Bristol.data2 <- Bristol.data1 %>% group_by(names) %>% summarise()
```
...
```{r}
#sample_0 <- match(Bristol.data1$names[Bristol.data1$Intervals == 0],sample_names(ps3ra))
ps3ra0 <- subset_samples(ps3ra,sample_names(ps3ra0) %in% Bristol.data1$names)
ps_glom <- tax_glom(ps3ra,"Family")

otu_tab <- as.data.frame(ps_glom@otu_table)
colnames(otu_tab) <- ps_glom@tax_table[,5]
otu_tab <- decostand(otu_tab, method = 'hellinger')

```

```{r}
sample_match <- match(Bristol.data1$names,rownames(otu_tab))
Bristol.data2 <- Bristol.data1[!is.na(sample_match),]
Bristol.data2$PD <- Bristol.data2$`PD Status`
Bristol.data2$PD[Bristol.data2$PD == 1] = "no"
Bristol.data2$PD[Bristol.data2$PD == 2] = "yes"
Bristol.data2$PD = as.factor(Bristol.data2$PD)

ps_rda2 <- otu_tab[na.omit(sample_match),]
#ps_rda2_0 <- ps_rda2[Bristol.data2$Intervals == 0,]
#Bristol_rda2_0 <- Bristol.data2[Bristol.data2$Intervals == 0,]

out_rda <- capscale(ps_rda2~`Bristol Score`+`PD Status`, Bristol.data2, distance = 'bray',add = T)
...
out_rda.site$PD <- Bristol.data2$PD
...
```{r}
matched.id <- c("18/121","18/106","18/129","18/124","18/204","18/231","18/263","19/009","19/013","18/111","18/069","18/128","18/046","18/203","18/220","18/251","19/004","19/012")
label <- c(1:9,1:9)
names(label) <- matched.id

ps3ra <- transform_sample_counts(ps3, function(x){x / sum(x)})
ps3ra@sam_data$Treatment[ps3ra@sam_data$Treatment == "Longitudinal"] = "Control"
ps3ra@sam_data$Treatment[ps3ra@sam_data$Treatment == "Duodopa"] = "LCIG"
ps3ra_phy <- tax_glom(ps3ra,taxrank = "Phylum")
ps3ra_phy <- subset_samples(ps3ra_phy, (id %in% matched.id)&(t == 0))
ps3ra_phy <- subset_samples(ps3ra_phy,!grepl("rpt",sample_names(ps3ra_phy)))

GPUF <- phyloseq::distance(ps3ra_phy, method = 'wunifrac')

matched <- label[match(ps3ra_phy@sam_data$id,matched.id)]
adTPPD_noconf_OTUs <- adonis2(GPUF ~ sample_data(ps3ra_phy)[["PD"]] + matched , perm = 9999, by = "margin")
adTPPD_noconf_OTUs

out.pcoa3ra <- ordinate(ps3ra_phy,  method = "PCoA", distance = GPUF)
evals <- out.pcoa3ra$values[,1]

sample_data <- ps3ra_phy@sam_data
sample_data$label <- label[sample_data$id]

df = cbind(out.pcoa3ra$vectors[,1:3],sample_data) %>% group_by(id) %>% arrange(t,.by_group = TRUE)
...
p12 <- ggplot(df) + geom_point(aes(x = Axis.1, y = Axis.2, color = PD))+ geom_path(aes(x = Axis.1, y = Axis.2, group = label), colour = "gray30" , alpha = 0.2, size = 1) + labs(x = paste0("PC1: ", 100*round(var_explained[1],digits = 4), "%"), y = paste0("PC2: ", 100*round(var_explained[2],digits = 4), "%")) +  theme_classic()  + stat_ellipse(aes(x = Axis.1, y = Axis.2, group = PD, color = PD), level = 0.9) + scale_color_manual(values =  brewer.pal(2, "Set2"),breaks = c("yes","no"),labels = c("PD","Control"))  + ggtitle("A. ")

p13 <- ggplot(df) + geom_path(aes(x = Axis.1, y = Axis.3, group = label), colour = "gray30" , alpha = 0.2, size = 1) + labs(x = paste0("PC1: ", 100*round(var_explained[1],digits = 4), "%"), y = paste0("PC3: ", 100*round(var_explained[3],digits = 4), "%")) +  theme_classic()  + stat_ellipse(aes(x = Axis.1, y = Axis.3, group = PD, color = PD), level = 0.9) + scale_color_manual(values =  brewer.pal(2, "Set2"),breaks = c("yes","no"),labels = c("PD","Control")) + geom_point(aes(x = Axis.1, y = Axis.3, color = PD)) + ggtitle("B. ")

p23 <- ggplot(df) + geom_path(aes(x = Axis.2, y = Axis.3, group = label), colour = "gray30" , alpha = 0.2, size = 1) + labs(x = paste0("PC2: ", 100*round(var_explained[2],digits = 4), "%"), y = paste0("PC3: ", 100*round(var_explained[3],digits = 4), "%")) +  theme_classic()  + stat_ellipse(aes(x = Axis.2, y = Axis.3, group = PD, color = PD), level = 0.9) +  geom_point(aes(x = Axis.2, y = Axis.3, color = PD)) + scale_color_manual(values =  brewer.pal(2, "Set2"),breaks = c("yes","no"),labels = c("PD","Control")) + ggtitle("C.")
```

```{r}
fig6 <- ggpubr::ggarrange(p12,p13,p23,ncol = 2,nrow = 2,common.legend = T)
ggexport(fig6, filename = "./stage1/v4/Fig10/figure10_BC_Phylum.pdf",width = fullpage, height = maxhi*0.75, units = "in")
tiff(filename = "./stage1/v4/Fig10/figure10_BC_Phylum.tiff" ,width=12, height=8, units="in", compression="lzw", res=150)
fig6
dev.off()
```
...
```{r}
ps3ra0 <- transform_sample_counts(ps3, function(x){x / sum(x)})

ps3ra0@sam_data$Treatment[ps3ra0@sam_data$Treatment == "Longitudinal"] = "Control"
ps3ra0@sam_data$Treatment[ps3ra0@sam_data$Treatment == "Duodopa"] = "LCIG"
ps3ra <- tax_glom(ps3ra0,taxrank = "Order")
#ps3ra <- ps3ra0
...
GPUF2 <- phyloseq::distance(ps3ra2, method = 'wunifrac')
out.pcoa3ra2 <- ordinate(ps3ra2,  method = "MDS", distance = GPUF2)
evals <- out.pcoa3ra2$values[,1]
...
GPUF3 <- phyloseq::distance(ps3ra3, method = 'wunifrac')
out.pcoa3ra3 <- ordinate(ps3ra3,  method = "MDS", distance = GPUF3)
...
```{r}
ps3ra4 <- subset_samples(ps3ra, ((t == 0))|(t == 4))

GPUF4 <- phyloseq::distance(ps3ra4, method = 'wunifrac')
out.pcoa3ra4 <- ordinate(ps3ra4,  method = "MDS", distance = GPUF4)

df3 = cbind(out.pcoa3ra4$vectors[,1:2],ps3ra4@sam_data) %>% group_by(id) %>% arrange(t,.by_group = TRUE) 
...
```{r}
ps3ra5 <- subset_samples(ps3ra, ((t == 2))|(t == 4))

GPUF5 <- phyloseq::distance(ps3ra5, method = 'wunifrac')
out.pcoa3ra5 <- ordinate(ps3ra5,  method = "MDS", distance = GPUF5)

df4 = cbind(out.pcoa3ra5$vectors[,1:2],ps3ra5@sam_data) %>% group_by(id) %>% arrange(t,.by_group = TRUE) 
var_explained = out.pcoa3ra5$values$Relative_eig[1:2]
...
df3$`TimePoint Subset` <- "t=0 vs. t=4"
df4$`TimePoint Subset` <- "t=2 vs. t=4"
df <- rbind(df1,df2,df3,df4)
df$`TimePoint Subset` <- factor(df$`TimePoint Subset`,levels = c("t=-2 vs. t=0","t=0 vs. t=2","t=2 vs. t=4","t=0 vs. t=4"))

fig4 <- ggplot(df) + geom_point(aes(x = Axis.1, y = Axis.2, color = Treatment, shape = t), size = 2) + geom_path(aes(x = Axis.1, y = Axis.2, group = id, colour = Treatment),  alpha = 0.5, size = 1) + labs(x = paste0("PC1 "), y = paste0("PC2 ")) +  theme_classic()  + stat_ellipse(aes(x = Axis.1, y = Axis.2, group = Treatment, color = Treatment, lty = Treatment), level = 0.9) + scale_color_manual(values = brewer.pal(3, "Set2")) + facet_wrap(~`TimePoint Subset`,ncol = 4,scales = "free")  + theme(legend.position = "top", panel.grid.minor = element_blank(), panel.grid.major =
element_blank())
...
ggexport(fig4, filename = "figure4_Order.pdf",width = 18, height =6, units = "in")
tiff(filename = "figure4_Order.tiff" ,width=16, height=6, units="in", compression="lzw", res=150)
fig4
dev.off()
```
...
p_Phy <- ggplot(psra_melt_Phy, aes(x = t, y = mean, fill = Phylum)) +
  ...
  legend.key.width = unit(4, "mm"),
  legend.text = element_text(size = 9),
  legend.margin = ggplot2::margin(r = 7, unit = "mm")) + ggtitle("A.")
...
p_Fam <- ggplot(psra_melt_Fam, aes(x = t, y = mean, fill = Family)) +
  ...
  legend.key.width = unit(4, "mm"),
  legend.text = element_text(size = 9),
  legend.margin = ggplot2::margin(r = 7, unit = "mm")) + ggtitle("C.")
...
p_Ord <- ggplot(psra_melt_Ord, aes(x = t, y = mean, fill = Order)) +
  ...
  legend.key.width = unit(4, "mm"),
  legend.text = element_text(size = 9),
  legend.margin = ggplot2::margin(r = 7, unit = "mm")) + ggtitle("B.")
...
p_Gen <- ggplot(psra_melt_Gen, aes(x = t, y = mean, fill = Genus)) +
  ...
  legend.key.width = unit(4, "mm"),
  legend.text = element_text(size = 9),
  legend.margin = ggplot2::margin(r = 7, unit = "mm"))+ ggtitle("D.")
```

```{r}
fig11 <- arrangeGrob(p_Phy,p_Ord,ncol = 1)
fig21 <- arrangeGrob(p_Fam,p_Gen,ncol = 1)
pdf("figure1.pdf")
grid.arrange(fig11)
grid.arrange(fig21)
...
fig1 <- arrangeGrob(p_Phy,p_Ord,p_Fam,p_Gen,ncol = 1)
...
grid.arrange(fig11)
ggsave(fig11, filename = "figure11.pdf",width = 8, height = 12, units = "in")
```
...
pOTUs <- function(psgen, t1, t2, idx = 6, iftab = F, ifotu = F){

  psgen1 <- subset_samples(psgen, ((t == t1)|(t == t2))&(Treatment == "LCIG"))
#  print(psgen1@sam_data$t)
#  print(psgen1@sam_data$Treatment)
  gen_table = t(as.data.frame(psgen1@otu_table))
#  print(dim(gen_table))
  if(ifotu){
    name_otu <- paste(psgen1@tax_table[,6],1:nrow(gen_table), sep = ":")
    colnames(psgen1@otu_table) = name_otu
    rownames(gen_table) = name_otu
  }else{
...
```{r}
logFC <- function(psgen, level= "Genus", t1,t2, ifotu = F){
  eps = 1e-10
  level1 <- rlang::sym(level)
  psgen1 <- subset_samples(psgen, ((t == t1)|(t == t2))&(Treatment == "LCIG"))
...
#  print(psgen1@sam_data$Treatment)
  ...
  if(ifotu){
#    psgen1_melt$OTU <- 
    ps_summary <- psgen1_melt %>% group_by(OTU, t) %>% summarise(mean = mean(Abundance)) %>% spread(key = t, value = mean)
  }else{
    ps_summary <- psgen1_melt %>% group_by(!!level1, t) %>% summarise(mean = mean(Abundance)) %>% spread(key = t, value = mean)
  }
  
  logFC_1 <- data.frame(taxa = ps_summary[,1],logFC = log2((ps_summary[,3]+eps)/(ps_summary[,2]+eps)))
...
```{r}
name_otu <- paste(psgen1@tax_table[,6],1:ncol(gen_table), sep = ":")
psgen <- subset_samples(ps3, Treatment != "Control")
psgen@sam_data$t = as.numeric(psgen@sam_data$t )

t1 = 0
t2 = -2

p_20 <- pOTUs(psgen = psgen, t1 = t1, t2 = t2,iftab = F, ifotu = T)

logFC_20 <- logFC(psgen = psgen, t1 = t1, t2 = t2, ifotu = T)
idx = match(names(p_20),logFC_20[,1])
logFC_20 = logFC_20[idx,2]

t2 = 2
p02 <- pOTUs(psgen = psgen, t1 = t1, t2 = t2, ifotu = T)
logFC02 <- logFC(psgen = psgen, t1 = t1, t2 = t2, ifotu = T)
idx = match(names(p02),logFC02[,1])
logFC02 = logFC02[idx,2]

t2 = 4
p04 <- pOTUs(psgen = psgen, t1 = t1, t2 = t2, ifotu = T)
logFC04 <- logFC(psgen = psgen, t1 = t1, t2 = t2, ifotu = T)
idx = match(names(p04),logFC04[,1])
logFC04 = logFC04[idx,2]
```

```{r}
p_Treatment_OTU <- data.frame(Genus = rep(name_otu,2), p1 = rep(p_20,2),logFC1 = rep(logFC_20,2), p2 = c(p02,p04),logFC2 = c(logFC02,logFC04), type = c(rep("-log(p-value): t=0 vs. t=2",length(p02)),rep("-log(p-value): t=0 vs. t=4",length(p04)))) 

```

```{r}
df_annotate <- p_Treatment_OTU[(p_Treatment_OTU$p2<0.1)|(p_Treatment_OTU$p1<0.1),]
df_annotate <- df_annotate[!duplicated(df_annotate$Genus),]
p1_OTU <- ggplot(p_Treatment_OTU) + geom_point(aes(x = -log(p1), y = -log(p2), color = type), size = 1.5) + geom_abline(aes(intercept = 0, slope = 1)) + theme_classic() + labs(x = "-log(p-value): t=0 vs. t=-2", y="-log(p-value): t=0 vs. t=2 or t=0 vs. t=4") + scale_color_manual(values = brewer.pal(2, "Dark2"), name = "Contrast Timepoint")+ geom_path(aes(x = -log(p1), y = -log(p2), group = Genus),colour = "gray50",alpha = 0.2) + geom_text(data = df_annotate,aes(x = -log(p1)+0.1, y = -log(p2) + 0.1, label = Genus))
p1_OTU
```

```{r}
ggplot(p_Treatment_OTU) + geom_point(aes(x = ))
```
...
```{r}
p_Treatment_Gen <- data.frame(Genus = name_otu, p1 = rep(p_20,2),logFC1 = rep(logFC_20,2), p2 = c(p02,p04),logFC2 = c(logFC02,logFC04), type = c(rep("-log(p-value): t=0 vs. t=2",length(p02)),rep("-log(p-value): t=0 vs. t=4",length(p04)))) 
...
df_annotate <- df_annotate[!duplicated(df_annotate$Genus),]
p_Treatment_Gen$alpha <- (p_Treatment_Gen$p2<0.1)|(p_Treatment_Gen$p1<0.1)
p_Treatment_Gen$alpha[p_Treatment_Gen$alpha == 1] = 1
p_Treatment_Gen$alpha[p_Treatment_Gen$alpha == 0] = 0.3

p1_Gen <- ggplot(p_Treatment_Gen) + geom_point(aes(x = -log(p1), y = -log(p2), color = type, alpha = anno), size = 1.5) + geom_abline(aes(intercept = 0, slope = 1)) + theme_classic() + labs(x = "-log(p-value): t=0 vs. t=-2", y="-log(p-value): t=0 vs. t=2 or t=0 vs. t=4") + scale_color_manual(values = brewer.pal(2, "Dark2"), name = "Contrast Timepoint")+ geom_path(aes(x = -log(p1), y = -log(p2), group = Genus),colour = "gray50",alpha = 0.2) + geom_text(data = df_annotate,aes(x = -log(p1)+0.1, y = -log(p2) + 0.1, label = Genus))
p1_Gen
```

```{r}
ggsave("OTU_LCIG.pdf", p1_Gen)
```
...
write.csv(df_gen,file = "volcano_family.csv")
```
...
p1 <- ggplot(data = D_shannon1) + geom_boxplot(aes(x = PD,y = value,fill = PD),width = 0.3) + labs(x = "",y = "shannon diversity") + theme_bw() + theme(axis.text.x = element_text(color = "black"),
  ...
  legend.key.width = unit(4, "mm"),
  legend.text = element_text(size = 9),
  legend.margin = ggplot2::margin(r = 7, unit = "mm")) + scale_fill_manual(values = c("#BF812D", "aquamarine4", "#9970AB")) + guides(fill = F) + ggtitle("A.")
p1

p2 <- ggplot(data = D_simpson1) + geom_boxplot(aes(x = PD,y = value,fill = PD),width = 0.3) + labs(x = "",y = "simpson diversity") + theme_bw() + theme(axis.text.x = element_text(color = "black"),
  strip.background = element_rect(fill = "white"),
  panel.grid.major.y = element_blank(),
  panel.grid.minor.y = element_blank(),
  panel.grid.major.x = element_line(color = "gray70", size = 0.4),
  ...
  legend.key.width = unit(4, "mm"),
  legend.text = element_text(size = 9),
  legend.margin = ggplot2::margin(r = 7, unit = "mm")) + scale_fill_manual(values = c("#BF812D", "aquamarine4", "#9970AB")) + guides(fill = F) + ggtitle("B.")
p2
```


```{r}
D_shannon2 <- D_shannon[,c("Treatment","shannon")]
D_shannon2$index <- "shannon"
#D_shannon1$PD <- as.character(D_shannon1$PD)
#D_shannon1$PD[D_shannon1$PD == "yes"] = "PD"
#D_shannon1$PD[D_shannon1$PD == "no"] = "Control"
colnames(D_shannon2)[2] = "value"
D_simpson2 <- D_simpson[,c("Treatment","simpson")]
D_simpson2$index <- "simpson"
#D_simpson1$PD <- as.character(D_simpson1$PD)
#D_simpson1$PD[D_simpson1$PD == "yes"] = "PD"
#D_simpson1$PD[D_simpson1$PD == "no"] = "Control"
colnames(D_simpson2)[2] = "value"
D_alpha1 <- rbind(D_shannon2,D_simpson2)
p1 <- ggplot(data = D_shannon2) + geom_boxplot(aes(x = Treatment,y = value,fill = Treatment),width = 0.3) + labs(x = "",y = "shannon diversity") + theme_bw() + theme(axis.text.x = element_text(color = "black"),
  strip.background = element_rect(fill = "white"),
  panel.grid.major.y = element_blank(),
  panel.grid.minor.y = element_blank(),
  panel.grid.major.x = element_line(color = "gray70", size = 0.4),
...
p2 <- ggplot(data = D_simpson2) + geom_boxplot(aes(x = Treatment,y = value,fill = Treatment),width = 0.3) + labs(x = "",y = "simpson diversity") + theme_bw() + theme(axis.text.x = element_text(color = "black"),
  strip.background = element_rect(fill = "white"),
  panel.grid.major.y = element_blank(),
  panel.grid.minor.y = element_blank(),
  panel.grid.major.x = element_line(color = "gray70", size = 0.4),
...
## Fig11
```{r}
psra_glom <- tax_glom(ps3ra, taxrank = "Genus")
psra_melt <- psmelt(psra_glom)
psra_check <- psra_melt %>% group_by(Genus) %>% summarise(mean = mean(Abundance)) %>% arrange(desc(mean))
top_gen <- factor(as.matrix(psra_check[1:10,1]),levels = as.character(as.vector(as.matrix(psra_check[1:10,1]))))

```

```{r}
psra_melt <- psra_melt %>% filter(Genus %in% top_gen)
psra_melt$Genus <- factor(psra_melt$Genus,levels = top_gen)
fig41 <- ggplot(psra_melt,aes(x = Genus, y = Abundance, color = t)) + geom_boxplot()+ xlab(NULL) +
  ylab("Mean relative abundance (%)") +
  scale_color_manual(values = brewer.pal(4, "Set2"), name = "t") +
  guides(fill = guide_legend(ncol = 1)) +
  ggtitle("D.Genera") +
  theme_bw(base_size = 11) +
  facet_grid(~ Treatment) +
  ...
  legend.key.width = unit(4, "mm"),
  legend.text = element_text(size = 9),
  legend.margin = ggplot2::margin(r = 7, unit = "mm")) + coord_flip()
```

```{r}
psra_glom <- tax_glom(ps3ra, taxrank = "Family")
psra_melt <- psmelt(psra_glom)
psra_check <- psra_melt %>% group_by(Family) %>% summarise(mean = mean(Abundance)) %>% arrange(desc(mean))
top_fam <- factor(as.matrix(psra_check[1:10,1]),levels = as.character(as.vector(as.matrix(psra_check[1:10,1]))))

```

```{r}
psra_melt <- psra_melt %>% filter(Family %in% top_fam)
psra_melt$Family <- factor(psra_melt$Family,levels = top_fam)
fig31 <- ggplot(psra_melt,aes(x = Family, y = Abundance, color = t)) + geom_boxplot()+ xlab(NULL) +
  ylab("Mean relative abundance (%)") +
  scale_color_manual(values = brewer.pal(4, "Set2"), name = "t") +
  guides(fill = guide_legend(ncol = 1)) +
  ggtitle("C.Family") +
...
```{r}
psra_glom <- tax_glom(ps3ra, taxrank = "Order")
psra_melt <- psmelt(psra_glom)
psra_check <- psra_melt %>% group_by(Order) %>% summarise(mean = mean(Abundance)) %>% arrange(desc(mean))
top_ord <- factor(as.matrix(psra_check[1:10,1]),levels = as.character(as.vector(as.matrix(psra_check[1:10,1]))))

```

```{r}
psra_melt <- psra_melt %>% filter(Order %in% top_ord)
psra_melt$Order <- factor(psra_melt$Order,levels = top_ord)
fig21 <- ggplot(psra_melt,aes(x = Order, y = Abundance, color = t)) + geom_boxplot()+ xlab(NULL) +
  ylab("Mean relative abundance (%)") +
  scale_color_manual(values = brewer.pal(4, "Set2"), name = "t") +
  guides(fill = guide_legend(ncol = 1)) +
  ggtitle("B.Order") +
...
```{r}
psra_glom <- tax_glom(ps3ra, taxrank = "Phylum")
psra_melt <- psmelt(psra_glom)
psra_check <- psra_melt %>% group_by(Phylum) %>% summarise(mean = mean(Abundance)) %>% arrange(desc(mean))
top_phy <- factor(as.matrix(psra_check[1:9,1]),levels = as.character(as.vector(as.matrix(psra_check[1:9,1]))))

```

```{r}
psra_melt <- psra_melt %>% filter(Phylum %in% top_phy)
psra_melt$Phylum <- factor(psra_melt$Phylum,levels = top_phy)
fig11 <- ggplot(psra_melt,aes(x = Phylum, y = Abundance, color = t)) + geom_boxplot()+ xlab(NULL) +
  ylab("Mean relative abundance (%)") +
  scale_color_manual(values = brewer.pal(4, "Set2"), name = "t") +
  guides(fill = guide_legend(ncol = 1)) +
  ggtitle("A.Phylum") +
...
```{r}
fig10 <- ggarrange(fig11,fig21,fig31,fig41,common.legend = T,ncol = 1)
ggsave(fig10, filename = "figure10.pdf",width = 8, height = 16, units = "in")
tiff(filename = "figure10.tiff", units="in",width = 16,height = 32, compression="lzw", res=150)
fig10
...
# Volcano
```{r}
psphy <- tax_glom(ps3, "Phylum")
psphy <- subset_samples(psphy)
psphy@sam_data$t = as.numeric(psphy@sam_data$t)

psphy1 <- subset_samples(psphy, (t == 0))
print(psphy1@sam_data$t)
phy_table = t(as.data.frame(psphy1@otu_table))
print(dim(phy_table))
rownames(phy_table) = psphy1@tax_table[,2]
meta_data = psphy1@sam_data
meta_data1 <- as.data.frame(as.matrix(meta_data))
#meta_data1$t <- factor(as.numeric(as.character(meta_data1$t)))
meta_data1$PD <- as.factor(as.character(meta_data1$PD))
...
phy_aldex <- aldex(phy_table, meta_data$PD, mc.samples=128, test="t", effect=TRUE,include.sample.summary=FALSE, denom="all", verbose=FALSE)
  

```

```{r}
color1 = rep("1",nrow(phy_aldex))
color1[(abs(phy_aldex$diff.btw)>1.5)&(phy_aldex$wi.ep<0.05)] = "2"
#df_anno = gen_aldex[(abs(gen_aldex$diff.btw)>1.5)&(gen_aldex$wi.ep<0.05),]
p1 <- ggplot(data = phy_aldex) + geom_point(aes(x = diff.btw, y = -log(wi.ep), color = color1)) + labs(x = "fold change", y = "-log(p)") + theme_classic() + scale_color_manual(values = c("gray30","red")) + theme(legend.position="none") + ggtitle("D.Phylum")

```

```{r}
df_gen1 <- gen_aldex1[,c("diff.btw","wi.ep","we.ep")]
write.csv(df_gen,file = "volcano_phylum.csv")
```

```{r}
psord <- tax_glom(ps3, "Order")
psord <- subset_samples(psord)
psord@sam_data$t = as.numeric(psord@sam_data$t)
...
ord_aldex2 <- aldex(ord_table, meta_data$PD, mc.samples=128, test="t", effect=TRUE,include.sample.summary=FALSE, denom="all", verbose=FALSE)
  

```

```{r}
color2 = rep("1",nrow(ord_aldex2))
color2[(abs(ord_aldex2$diff.btw)>1.5)&(ord_aldex2$wi.ep<0.05)] = "2"
#df_anno = gen_aldex[(abs(gen_aldex$diff.btw)>1.5)&(gen_aldex$wi.ep<0.05),]
p2 = ggplot(data = ord_aldex2) + geom_point(aes(x = diff.btw, y = -log(wi.ep), color = color2)) + labs(x = "fold change", y = "-log(p)") + theme_classic() + scale_color_manual(values = c("gray30","red")) + theme(legend.position="none") + ggtitle("C.Order")

```

```{r}
df_gen <- gen_aldex[,c("diff.btw","wi.ep","we.ep")]
write.csv(df_gen,file = "volcano_order.csv")
```
...
gen_table = t(as.data.frame(psgen1@otu_table))
print(dim(gen_table))
rownames(gen_table) = psgen1@tax_table[,5]
meta_data = psgen1@sam_data
meta_data1 <- as.data.frame(as.matrix(meta_data))
#meta_data1$t <- factor(as.numeric(as.character(meta_data1$t)))
meta_data1$PD <- as.factor(as.character(meta_data1$PD))
...
```{r}
color3 = rep("1",nrow(gen_aldex))
color3[(abs(gen_aldex$diff.btw)>1.5)&(gen_aldex$wi.ep<0.05)] = "2"
#df_anno = gen_aldex[(abs(gen_aldex$diff.btw)>1.5)&(gen_aldex$wi.ep<0.05),]
p3 = ggplot(gen_aldex) + geom_point(aes(x = diff.btw, y = -log(wi.ep), color = color3)) + labs(x = "fold change", y = "-log(p)") + theme_classic() + scale_color_manual(values = c("gray30","red")) + theme(legend.position="none") + ggtitle("B.Family")
...
```{r}
color4 = rep("1",nrow(gen_aldex))
color4[(abs(gen_aldex$diff.btw)>1.5)&(gen_aldex$wi.ep<0.05)] = "2"
#df_anno = gen_aldex[(abs(gen_aldex$diff.btw)>1.5)&(gen_aldex$wi.ep<0.05),]
p4 = ggplot(gen_aldex) + geom_point(aes(x = diff.btw, y = -log(wi.ep), color = color4)) + labs(x = "fold change", y = "-log(p)") + theme_classic() + scale_color_manual(values = c("gray30","red")) + theme(legend.position="none") + ggtitle("A.Genus")

```

```{r}
df_gen <- gen_aldex[,c("diff.btw","wi.ep","we.ep")]
write.csv(df_gen,file = "volcano_genus.csv")
```


```{r}
fig5 <- ggpubr::ggarrange(p4,p3,p2,p1,ncol = 2,nrow = 2,common.legend = F)
ggexport(fig5, filename = "figure5.pdf",width = fullpage, height = maxhi*0.75, units = "in")
tiff(filename = "figure5.tiff" ,width=12, height=8, units="in", compression="lzw", res=150)
fig5
dev.off()
```
...
ps30 <- subset_samples(ps3, (t==0)&(id %in% matched.id))
Diet <- as.data.frame(cbind(ps30@sam_data$Tot_Fat_g_per_day*9,ps30@sam_data$Tot_Protein_g_per_day*4,ps30@sam_data$Tot_Avail_Carbs_g_per_day*4))
colnames(Diet) <- c("Fat","Protein","Carb")
rownames(Diet) <- sample_names(ps30)
Diet$id <- ps30@sam_data$id
Diet <- Diet%>% group_by(id) %>% summarise_all(mean)
Diet <- na.omit(Diet)
Diet_id <- Diet$id
Diet <- Diet[,-1]
Diet_percent <- Diet/rowSums(Diet)
rownames(Diet) <- Diet_id
rownames(Diet_percent) <- Diet_id

Diet_clinical <- sample_data(ps30)[ps30@sam_data$id %in% Diet_id,]
Diet_clinical <- Diet_clinical[!duplicated(Diet_clinical$id),]
```

```{r}
Diet_clinical <- Diet_clinical[match(Diet_id,Diet_clinical$id),]

Diet_data <- cbind(Diet_percent,Diet_clinical)
Diet_data$label <- label[rownames(Diet_data)]
Diet_p <-ggplot(Diet_data) + geom_point(aes(x = Protein, y = Carb, shape = `Constipation.ID...170`, color = PD)) + theme_bw() + scale_color_manual(values = brewer.pal(3, "Set2"))  + geom_abline(aes(intercept = 1, slope = -1)) + geom_path(aes(x = Protein, y = Carb, group = label),colour = "gray30",  alpha = 0.5, size = 1) + scale_shape_discrete(name = "Constipation")
```

```{r}
ggsave(Diet_p,filename = "Fig8_v2.pdf",width = 8,height = 8,units = "in")
```
...
```{r}
eps = 1e-9
ps3ra0 <- subset_samples(ps3ra, (t == 0)&(sample_names(ps3ra) != "19-201-t-0"))
psgen1 <- tax_glom(ps3ra0,"Genus")
...
Diet <- as.matrix(clinical[,c("Tot_Protein_g_per_day","Tot_Fat_g_per_day","Tot_Fibre_g_per_day","Tot_Avail_Carbs_g_per_day")])

clinical1 <- clinical[,c("Age","Sex","BMI","Height (cm)","Weight (kg)","Constipation ID...168","Bristol Score","CRP","Total Cholesterol","HbA1c...240","LDL","HDL","Glucose","Trigs","Albumin","ESR")]

clinical1$Sex = ifelse(clinical1$Sex == "Male",1,0)
clinical1$`Constipation ID...168` = ifelse(clinical1$Sex == "yes",1,0)

clinical1 <- as.matrix(clinical1)

Y <- as.factor(clinical$PD)
```

```{r}
X <- list(Genus = as.matrix(psgen1.df_log), Diet = scale(Diet), clinical = scale(clinical1))
    

list.keepX <- list(Genus = c(5,15), Diet = c(2,4), clinical = c(2,7))
```
...
# Fig1 v4
```{r}
ps3ra0 <- subset_samples(ps3ra,t == 0)

psra_melt_Fam <- psmelt(tax_glom(ps3ra0,"Family"))

psra_melt_Fam1 <- psra_melt_Fam %>% group_by(id,Family) %>% summarise(Abundance = mean(Abundance))
psra_check <- psra_melt_Fam %>% group_by(Family) %>% summarise(Abundance = mean(Abundance)) %>% arrange(desc(Abundance))

psra_Rel <- psra_melt_Fam1[as.character(psra_melt_Fam1$Family) %in% as.character(as.vector(as.matrix(psra_check[1:9,1]))),]
psra_other <- psra_Rel %>% group_by(id)  %>% summarise(Abundance = 1 - sum(Abundance))
psra_other$Family <- "other taxa"
psra_other <- psra_other[,c(1,3,2)]

psra_Rel1 <- rbind(as.data.frame(psra_Rel),as.data.frame(psra_other)) %>% arrange(desc(id))
psra_Rel1$Treatment <- psra_melt_Fam$Treatment[match(psra_Rel1$id,psra_melt_Fam$id)]

psra_Rel1 <- as.data.frame(psra_Rel1)
psra_Rel1$id <- paste(psra_Rel1$Treatment,psra_Rel1$id,sep = "")
psra_Rel1$id_num <- as.numeric(as.factor(psra_Rel1$id))
p <- ggplot(psra_Rel1) + geom_area(aes(x = id_num, y = Abundance, fill = Family,color = Treatment)) + theme_bw() + scale_fill_manual(values = brewer.pal(10, "RdYlBu")) + theme(panel.grid.major =element_blank(), 
        panel.grid.minor = element_blank(),
        panel.background = element_blank(),
        panel.border = element_blank(),
        axis.text.x = element_blank()) + labs(x = "", y = "mean relative abundance") + scale_color_discrete(name = "")
p
```

```{r}
ggsave(filename = "./stage1/v4/Figure1_v3.pdf",plot = p_Fam,width = 8,height = 4,units = "in")
```
...
```{r}
temp <- psra_Rel1[psra_Rel1$Family == "Bacteroidaceae",]
temp1 <- temp[temp$Treatment == "Control",]
temp1 <- temp1[order(temp1$Abundance,decreasing = T),]
temp1$id_num <- 1:nrow(temp1)

temp2 <- temp[temp$Treatment == "DBS",]
temp2 <- temp2[order(temp2$Abundance,decreasing = T),]
temp2$id_num <- (nrow(temp1)+1):(nrow(temp1)+nrow(temp2))

temp3 <- temp[temp$Treatment == "LCIG",]
temp3 <- temp3[order(temp3$Abundance,decreasing = T),]
temp3$id_num <- (nrow(temp1)+nrow(temp2) + 1):(nrow(temp1)+nrow(temp2)+nrow(temp3))

temp <- rbind(temp1,temp2,temp3)
psra_Rel1$id_num <- temp$id_num[match(psra_Rel1$id,temp$id)]
```
...
```{r}
p_Fam <- ggplot(psra_Rel1, aes(x = id, y = Abundance, fill = Family)) +
  geom_bar(stat = "identity", position = "stack") +
  xlab(NULL) +
  ylab("Mean relative abundance (%)") +
  scale_fill_manual(values = bccolsF, name = "Family") +
...
```{r}
ps3ra_DBS <- subset_samples(ps3ra, Treatment=="DBS")

psra_melt_Fam <- psmelt(tax_glom(ps3ra_DBS,"Family"))

psra_melt_Fam1 <- psra_melt_Fam %>% group_by(id,Family,t) %>% summarise(Abundance = mean(Abundance))
psra_check <- psra_melt_Fam %>% group_by(Family) %>% summarise(Abundance = mean(Abundance)) %>% arrange(desc(Abundance))

psra_Rel <- psra_melt_Fam1[as.character(psra_melt_Fam1$Family) %in% as.character(as.vector(as.matrix(psra_check[1:9,1]))),]
psra_other <- psra_Rel %>% group_by(id,t)  %>% summarise(Abundance = 1 - sum(Abundance))
psra_other$Family <- "other taxa"
psra_other <- psra_other[,c(1,4,2,3)]

psra_Rel1 <- rbind(as.data.frame(psra_Rel),as.data.frame(psra_other)) %>% arrange(desc(id))
#psra_Rel1$Treatment <- psra_melt_Fam$Treatment[match(psra_Rel1$id,psra_melt_Fam$id)]

psra_Rel1 <- as.data.frame(psra_Rel1)
psra_Rel1$id <- paste(psra_Rel1$id,psra_Rel1$t,sep = "")
psra_Rel1$id_num <- as.numeric(as.factor(psra_Rel1$id))
p <- ggplot(psra_Rel1) + geom_area(aes(x = id_num, y = Abundance, fill = Family)) + theme_bw() + scale_fill_manual(values = brewer.pal(10, "RdYlBu")) + theme(panel.grid.major =element_blank(), 
        panel.grid.minor = element_blank(),
        panel.background = element_blank(),
        panel.border = element_blank(),
        axis.text.x = element_blank()) + labs(x = "", y = "mean relative abundance") + scale_color_discrete(guide = F)
p
```

```{r}
ggsave(p_Fam,filename = "./stage1/v4/Figure1.2_v2.pdf",width = 8,height = 4,units = "in")
```


```{r}
ps3ra_LCIG <- subset_samples(ps3ra,Treatment=="LCIG")

psra_melt_Fam <- psmelt(tax_glom(ps3ra_LCIG,"Family"))
...
```{r}
ggsave(p,file = "./stage1/v4/Figure1.2_v1_LCIG.pdf",width = 8,height = 4,units = "in")
```
...
ps3 <- prune_samples(sample_names(ps2) != "18-203-t-2",ps2)
ps3ra <- transform_sample_counts(ps3, function(x){x / sum(x)})
ps3ra_phy <- tax_glom(ps3ra,taxrank = "Family")
ps3ra@sam_data$Treatment[ps3ra@sam_data$Treatment == "Longitudinal"] = "Control"
ps3ra@sam_data$Treatment[ps3ra@sam_data$Treatment == "Duodopa"] = "LCIG"
ps3ra_phy <- subset_samples(ps3ra_phy,Treatment != "Longitudinal")
#ps3ra <- subset_samples(ps3ra, t %in% c(0))
#ps3ralog <- transform_sample_counts(ps3ra, function(x){log(x + eps)})
...
df = cbind(out.pcoa3ra$vectors[,1:3],ps3ra_phy@sam_data) %>% group_by(id) %>% arrange(t,.by_group = TRUE)
var_explained = out.pcoa3ra$values$Relative_eig[1:3]
...
df_20 <- df[df$t %in% c(-2,0),]
p_20 <- ggplot(df_20) + geom_point(aes(x = Axis.1, y = Axis.2, color = Treatment, shape = t), size = 2) + geom_path(aes(x = Axis.1, y = Axis.2, group = id, colour = Treatment),  alpha = 0.5, size = 1) + labs(x = paste0("PC1: ", 100*round(var_explained[1],digits = 4), "%"), y = paste0("PC2: ", 100*round(var_explained[2],digits = 4), "%")) +  theme_classic()  + stat_ellipse(aes(x = Axis.1, y = Axis.2, group = Treatment, color = Treatment, lty = Treatment), level = 0.9) + scale_color_manual(values = brewer.pal(3, "Set2")) + ggtitle("A.")

df02 <- df[df$t %in% c(0,2),]
p02 <- ggplot(df02) + geom_point(aes(x = Axis.1, y = Axis.2, color = Treatment, shape = t), size = 2) + geom_path(aes(x = Axis.1, y = Axis.2, group = id, colour = Treatment),  alpha = 0.5, size = 1) + labs(x = paste0("PC1: ", 100*round(var_explained[1],digits = 4), "%"), y = paste0("PC2: ", 100*round(var_explained[2],digits = 4), "%")) +  theme_classic()  + stat_ellipse(aes(x = Axis.1, y = Axis.2, group = Treatment, color = Treatment, lty = Treatment), level = 0.9) + scale_color_manual(values = brewer.pal(3, "Set2")) + ggtitle("B.")

df04 <- df[df$t %in% c(0,4),]
p04 <- ggplot(df04) + geom_point(aes(x = Axis.1, y = Axis.2, color = Treatment, shape = t), size = 2) + geom_path(aes(x = Axis.1, y = Axis.2, group = id, colour = Treatment),  alpha = 0.5, size = 1) + labs(x = paste0("PC1: ", 100*round(var_explained[1],digits = 4), "%"), y = paste0("PC2: ", 100*round(var_explained[2],digits = 4), "%")) +  theme_classic()  + stat_ellipse(aes(x = Axis.1, y = Axis.2, group = Treatment, color = Treatment, lty = Treatment), level = 0.9) + scale_color_manual(values = brewer.pal(3, "Set2")) + ggtitle("C.")

df24 <- df[df$t %in% c(2,4),]
p24 <- ggplot(df24) + geom_point(aes(x = Axis.1, y = Axis.2, color = Treatment, shape = t), size = 2) + geom_path(aes(x = Axis.1, y = Axis.2, group = id, colour = Treatment),  alpha = 0.5, size = 1) + labs(x = paste0("PC1: ", 100*round(var_explained[1],digits = 4), "%"), y = paste0("PC2: ", 100*round(var_explained[2],digits = 4), "%")) +  theme_classic()  + stat_ellipse(aes(x = Axis.1, y = Axis.2, group = Treatment, color = Treatment, lty = Treatment), level = 0.9) + scale_color_manual(values = brewer.pal(3, "Set2")) + ggtitle("D.")
```

```{r}
ggpubr::ggarrange(p_20,p02,p04,p24,ncol = 2,nrow = 2,common.legend = T)
```
 

# Optimal Rotation
```{r}
Rotation <- function(U,V){
  U <- as.matrix(U)
  V <- as.matrix(V)
  H <- t(U) %*% V
  H_eig <- svd(H)
  R <- H_eig$v %*% diag(rep(1,min(nrow(H),ncol(H)))) %*% t(H_eig$u)
  return(R)
}
```

```{r}
opt_Rotation <- function(Ref, to_Rot, ps3ra2 = ps3ra2, ps3ra3 = ps3ra3){
  Ref0 <- Ref
  Ref1 <- Ref0[,c("Axis.1","Axis.2")]
 # print(length(ps3ra2@sam_data$Treatment == "Control"))
  Ref11 <- Ref1[ps3ra2@sam_data$Treatment == "Control",]
  to_Rot0 <- to_Rot
  to_Rot1 <- to_Rot0[,c("Axis.1","Axis.2")]
 # print(length(ps3ra3@sam_data$Treatment == "Control"))
  to_Rot11 <- to_Rot1[ps3ra3@sam_data$Treatment == "Control",]
  R <- Rotation(Ref11,to_Rot11)
  to_Rot1 <- as.matrix(to_Rot1) %*% R
  colnames(to_Rot1) <- c("Axis.1","Axis.2")
  to_Rot1 <- as.data.frame(to_Rot1)
  return(to_Rot1)
}
```

```{r}
df11 <- df1[,c("Axis.1","Axis.2")]
df1[,c("Axis.1","Axis.2")] <- df11

df21 <- df2[,c("Axis.1","Axis.2")]
df22 <- opt_Rotation(df11,df21,ps3ra2 = ps3ra2,ps3ra3 = ps3ra3)
df2[,c("Axis.1","Axis.2")] <- df22

df31 <- df3[,c("Axis.1","Axis.2")]
df32 <- opt_Rotation(df11,df31,ps3ra2 = ps3ra2,ps3ra3 = ps3ra4)
df3[,c("Axis.1","Axis.2")] <- df32
...
df$`TimePoint Subset` <- factor(df$`TimePoint Subset`,levels = c("t=-2 vs. t=0","t=0 vs. t=2","t=0 vs. t=4"))

fig4 <- ggplot(df) + geom_point(aes(x = Axis.1, y = Axis.2, color = Treatment, shape = t), size = 2) + geom_path(aes(x = Axis.1, y = Axis.2, group = id, colour = Treatment),  alpha = 0.5, size = 1) + labs(x = paste0("PC1 "), y = paste0("PC2 ")) +  theme_classic()  + stat_ellipse(aes(x = Axis.1, y = Axis.2, group = Treatment, color = Treatment, lty = Treatment), level = 0.9) + scale_color_manual(values = brewer.pal(3, "Set2")) + facet_wrap(~`TimePoint Subset`,ncol = 4,scales = "fixed")  + theme(legend.position = "top", panel.grid.minor = element_blank(), panel.grid.major =
element_blank())
```
...
```{r}
X1 <- mvrnorm(n = 20,mu = rep(1,50),Sigma = 0.5*diag(rep(1,50))+0.5*matrix(1,nrow = 50,ncol = 50))
X2 <- mvrnorm(n = 20,mu = rep(2,50),Sigma = 0.5*diag(rep(1,50))+0.5*matrix(1,nrow = 50,ncol = 50))
X3 <- mvrnorm(n = 20,mu = rep(-1,50),Sigma = 0.5*diag(rep(1,50))+0.5*matrix(1,nrow = 50,ncol = 50))
```

```{r}
Xa <- rbind(X1, X2)
Xb <- rbind(X1, X3)
X <- rbind(cbind(Xa, matrix(0, nrow = 40,ncol = 50)),cbind(matrix(0, nrow = 40,ncol = 50),Xb))
```
...
```{r}
tax_tab <- as.data.frame(ps3@tax_table) 
```
