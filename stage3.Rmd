...
var_explained1 = out.pcoa3ra1$values$Relative_eig[1:2]
#plot_ordination(ps3ra2, out.pcoa3ra3, color = "Cohort",
#                  shape = "t") +
#  labs(col = "Cohort", shape = "Time")+
#  coord_fixed(sqrt(evals[2] / evals[1])) + theme_classic()
...
var_explained2 = out.pcoa3ra2$values$Relative_eig[1:2]
...
var_explained3 = out.pcoa3ra3$values$Relative_eig[1:2]
```
...
fig5A <- ggplot(df1)+ geom_point(aes(x = Axis.1, y = Axis.2, color = Treatment), size = 2) + labs(x = paste0("PC1: ", 100*round(var_explained1[1],digits = 4), "%"), y = paste0("PC2: ", 100*round(var_explained1[2],digits = 4), "%"), title = "A.t = 0") +  theme_classic()  + stat_ellipse(aes(x = Axis.1, y = Axis.2, group = Treatment, color = Treatment, lty = Treatment), level = 0.9) + scale_color_manual(values = brewer.pal(4, "Set2"))

fig5B <- ggplot(df2)+ geom_point(aes(x = Axis.1, y = Axis.2, color = Treatment), size = 2) + labs(x = paste0("PC1: ", 100*round(var_explained2[1],digits = 4), "%"), y = paste0("PC2: ", 100*round(var_explained2[2],digits = 4), "%"), title = "B.t = 6") +  theme_classic()  + stat_ellipse(aes(x = Axis.1, y = Axis.2, group = Treatment, color = Treatment, lty = Treatment), level = 0.9) + scale_color_manual(values = brewer.pal(4, "Set2"))

fig5C <- ggplot(df3)+ geom_point(aes(x = Axis.1, y = Axis.2, color = Treatment), size = 2) + labs(x = paste0("PC1: ", 100*round(var_explained3[1],digits = 4), "%"), y = paste0("PC2: ", 100*round(var_explained3[2],digits = 4), "%"), title = "C.t = 12") +  theme_classic()  + stat_ellipse(aes(x = Axis.1, y = Axis.2, group = Treatment, color = Treatment, lty = Treatment), level = 0.9) + scale_color_manual(values = brewer.pal(4, "Set2"))


fig50 <- ggpubr::ggarrange(fig5A, fig5B, fig5C, nrow = 1, ncol = 3, common.legend = T)
```
...
```{r}
fig_syn <- ggpubr::ggarrange(p1,p2,p3,ncol = 3,nrow = 1, common.legend = T)
fig_syn
ggsave(filename = "fig_syn_fs.pdf",fig_syn,width = 18, height=5, units="in")
```
...
```{r}
CV_pred <- function(df, y = y, method = "all"){
  ...
  for(i in 1:n){
    ...
    if(method == "all"){
      ...
      pred[i,1] <- predict(object = lasso_fit, newx = X_test, type = "response", s = "lambda.min") 
      ...
      pred[i,3] <- predict(rf_fit, X_test, type="prob")[,"1"]
    
      pred[i,4] <- y_test
    }else if(method == "rf"){
      
      rf_fit <- randomForest(x = X_train, y = y_train, ntree = 500)
      pred[i,1] <- predict(rf_fit, X_test, type="prob")[,"1"]
  ...
  if(method == "all"){
    ...
    colnames(pred) <- c("random forest", "actual")
  }
  
  return(pred)
}

CV_ROC <- function(ps_ra_D12){
  ...
  psphy_df <- psphy@otu_table
  colnames(psphy_df) <- psphy@tax_table[,2]
  psphy_df_log <- trans_comp(abs(psphy_df), eps = 1e-5, method = "clr")
  psphy_df_asin <- trans_comp(abs(psphy_df), eps = 1e-5, method = "asin")
  
  psord <- tax_glom(ps_test, taxrank = "Order")
  #psord_ra <- transform_sample_counts(psord, function(x){x / sum(x)})
  psord_df <- psord@otu_table
  colnames(psord_df) <- psord@tax_table[,4]
  psord_df_log <- trans_comp(abs(psord_df), eps = 1e-5, method = "clr")
  psord_df_asin <- trans_comp(abs(psord_df), eps = 1e-5, method = "asin")
  
  psfam <- tax_glom(ps_test, taxrank = "Family")
  #psfam_ra <- transform_sample_counts(psfam, function(x){x / sum(x)})
  psfam_df <- psfam@otu_table
  colnames(psfam_df) <- psfam@tax_table[,5]
  psfam_df_log <- trans_comp(abs(psfam_df), eps = 1e-5, method = "clr")
  psfam_df_asin <- trans_comp(abs(psfam_df), eps = 1e-5, method = "asin")
  
  psgen <- tax_glom(ps_test, taxrank = "Genus")
  #psgen_ra <- transform_sample_counts(psgen, function(x){x / sum(x)})
  psgen_df <- psgen@otu_table
  colnames(psgen_df) <- psgen@tax_table[,6]
  psgen_df_log <- trans_comp(abs(psgen_df), eps = 1e-5, method = "clr")
  psgen_df_asin <- trans_comp(abs(psgen_df), eps = 1e-5, method = "asin")
  
  #psotu_ra <- transform_sample_counts(ps_test, function(x){x / sum(x)})
  psotu_df <- ps_test@otu_table
  colnames(psotu_df) <- paste(ps_test@tax_table[,6], paste("o.", 1:ncol(psotu_df), sep = ""), sep = ":")
  psotu_df_log <- trans_comp(abs(psotu_df), eps = 1e-5, method = "clr")
  psotu_df_asin <- trans_comp(abs(psotu_df), eps = 1e-5, method = "asin")
  
  X_list <- list(scale(psphy_df), psphy_df_log, psphy_df_asin, scale(psord_df), psord_df_log, psord_df_asin, scale(psfam_df), psfam_df_log, psfam_df_asin, scale(psgen_df), psgen_df_log, psgen_df_asin, scale(psotu_df), psotu_df_log, psotu_df_asin)
  
  y <- as.numeric(as.factor(ps_test@sam_data$type)) - 1
...
roc_method <- function(mat, method = "all"){
  ...
  if(method == "all"){
    ...
    return(rocpdf(mat, method = "random forest"))
  }
}
```
...
df_gen <- roc_res[[11]]$df
df_gen$tax <- "Genus"

df_asv <- roc_res[[14]]$df
df_asv$tax <- "ASV"

df_roc_tax <- rbind(df_phy, df_ord, df_fam, df_gen, df_asv)
df_auc_tax <- rbind(roc_res[[2]]$auc, roc_res[[5]]$auc, roc_res[[8]]$auc, roc_res[[11]]$auc, roc_res[[14]]$auc)
```
...
caption_rf <- paste0("AUC: ", round(auc_phy_rf_test, 2),"(Phylum), ", round(auc_ord_rf_test, 2),"(Order), ", round(auc_fam_rf_test, 2), "(Family), ",round(auc_gen_rf_test, 2), "(Genus), ",round(auc_otu_rf_test, 2), "(ASV)")


p_rf_test <- ggplot(data = roc_rf_test) + geom_path(aes(x = x,y = y,color = taxa),size = 1)  +
        labs(title = "LOOCV-ROC curve of RF model on different taxanomic level:", caption = caption_rf) + 
        xlab("1 - Specificity") +
        ylab("Sensitivity") +
        theme(plot.title = element_text(size = 10)) + theme_bw() + geom_abline(slope = 1,intercept = 0)
p_rf_test
...
auc_res_rf <- array(0, dim = c(length(LED_list), length(UPDRS_list), 5))
...
for(i in 1:length(LED_list)){
  ...
  #psfam_ra <- transform_sample_counts(psfam, function(x){x / sum(x)})
    ...
    #psotu_ra <- transform_sample_counts(ps_test, function(x){x / sum(x)})
    psotu_df <- ps_ra_D12@otu_table
    colnames(psotu_df) <- paste(ps_test@tax_table[,6], paste("o.", 1:ncol(psotu_df), sep = ""), sep = ":")
    psotu_df_log <- trans_comp(abs(psotu_df), eps = 1e-5, method = "clr")
    
    
    X_list <- list(psphy_df_log, psord_df_log, psfam_df_log, psgen_df_log, psotu_df_log)
    
    y <- as.numeric(as.factor(group)) - 1
    
    CV_pred_func <- purrr::partial(CV_pred, y = y, method = "rf")
    
    pred_list <- lapply(X_list, CV_pred_func)
    roc_rf <- purrr::partial(roc_method, method = "rf")
    roc_list <- lapply(pred_list, roc_rf)

    auc_res_rf[i,j,] <- c(roc_list[[1]]$auc, roc_list[[2]]$auc, roc_list[[3]]$auc, roc_list[[4]]$auc, roc_list[[5]]$auc)
  }
}
```

```{r}
dimnames(auc_res_rf) <- list(LED_list, UPDRS_list, c("Phylum", "Order","Family", "Genus", "ASV"))
rownames(auc_res_rf) <- LED_list
colnames(auc_res_rf) <- UPDRS_list
auc_plot_rf <- melt(auc_res_rf[,,5])

auc_plot_rf_temp <- apply(auc_res_rf, 3, melt)
...
p1 = directlabels::direct.label(p1,"bottom.pieces") + ggtitle("AUC value with LED and UPDRS threshold on ASV level") + xlab("LED Difference") + ylab("UPDRS Difference")
```
...
CV_split <- function(psfam_df_log){
  ...
  for(i in 1:n){
    ...
    for(j in 1:8){
      ...
      if(sel){
        pred_temp <- predict(rf1, X_test, type = "prob")[,2]
      }else{
        pred_temp <- predict(rf2, X_test, type = "prob")[,2]
      }
...
```{r}
roc_split <- roc_method1(pred_split)
```
...
roc_split_df$model <- factor(roc_split_df$model, levels = c("Carb", "Fat", "Sugar", "Fiber", "Fratio", "Cratio","Pratio", "Protein", "Microbiome"))
```
